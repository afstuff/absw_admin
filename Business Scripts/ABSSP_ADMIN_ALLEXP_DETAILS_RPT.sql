...
GO
IF OBJECT_ID (N'[dbo].ABSSP_ADMIN_ALLEXP_DETAILS_RPT', N'P') IS NOT NULL
begin
   DROP PROCEDURE [dbo].ABSSP_ADMIN_ALLEXP_DETAILS_RPT;
   IF  EXISTS (	SELECT * FROM sys.objects 
				WHERE object_id = OBJECT_ID(N'[dbo].[ABSSP_ADMIN_ALLEXP_DETAILS_RPT]') 
				AND type in (N'P')
			  )
		Print '<<< Failed Dropping  Procedure [ABSSP_ADMIN_ALLEXP_DETAILS_RPT] created !!! >>>'
	else
		Print '<<< Procedure [dbo].[ABSSP_ADMIN_ALLEXP_DETAILS_RPT] Dropped >>>'
end   
else
	Print '<<< !!! Error Procedure [dbo].[ABSSP_ADMIN_ALLEXP_DETAILS_RPT] does Not exist >>>' ;   
GO
create proc  [dbo].ABSSP_ADMIN_ALLEXP_DETAILS_RPT
(		
...
IF  EXISTS (SELECT * FROM sys.objects 
			WHERE object_id = OBJECT_ID(N'[dbo].[ABSSP_ADMIN_ALLEXP_DETAILS_RPT]') 
			AND type in (N'P'))
	Print '<<< Success Procedure [dbo].[ABSSP_ADMIN_ALLEXP_DETAILS_RPT] created !!! >>>'
else
	Print '<<< !!! Error Procedure [dbo].[ABSSP_ADMIN_ALLEXP_DETAILS_RPT] Not created >>>'
GO		  